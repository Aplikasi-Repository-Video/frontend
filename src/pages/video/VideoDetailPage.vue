    ...
    <main class="flex-1 bg-[#0f0b1d] overflow-y-auto px-6 pb-6">
      ...
      <div v-else class="max-w-4xl mx-auto">
        ...
        <div class="mt-4">
          ...
          >
            <p v-html="isExpanded ? video?.description : truncatedDescription"></p>
            <button
...
async function reportWatchHistory() {
  if (!video.value || !videoPlayer.value || hasReported) return

  const watched = Math.floor(videoPlayer.value.currentTime)
  const total = Math.floor(videoPlayer.value.duration)
  ...
  if (watched >= threshold && !alreadyWatched(video.value.id)) {
    ...
    if (user && typeof user === 'object' && user.id) {
      ...
      payload.guest_id = localStorage.getItem('guest_id') || generateGuestId()
    }
...
onMounted(() => {
  ...
  watch(
    ...
    (player) => {
      if (!player) return
      player.addEventListener('pause', reportWatchHistory)
      player.addEventListener('ended', reportWatchHistory)
    },
    { immediate: true }
  )
})
...
onBeforeUnmount(() => {
  reportWatchHistory() // kirim sebelum user keluar
})
</script>
